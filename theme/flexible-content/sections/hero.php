...
<?php if (!(is_account_page() && !is_user_logged_in())) : ?>
    <div class="relative w-full h-[300px] md:h-[600px] <?php if ($lowerHeight) : ?> md:!h-[400px] <?php endif; ?> flex flex-col items-center justify-center">
        ...
        <div class="relative z-0 flex flex-col items-center justify-center container">
            ...
            <?php if ($hero_text) : ?>
                <h1 class="<?php if ($display_logo) : ?>text-3xl md:text-[46px]<?php else : ?>text-5xl md:text-[46px] max-w-[760px] [.woocommerce-cart_&]:max-w-[880px] md:leading-[55px]<?php endif; ?> text-bold text-white font-bold text-center"><?php echo str_replace("®","<sup>®</sup>",$hero_text); ?></h1>
            <?php endif; ?>
        </div>

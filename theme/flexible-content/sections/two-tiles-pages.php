        ...
        <?php if ($tiles_list) : ?>
            <div class="tiles-wrapper flex flex-col lg:flex-row gap-10">
                <?php foreach ($tiles_list as $tile) : ?>
                    <div class="tile group bg-white relative h-[340px] !flex items-center justify-center flex-col lg:!basis-1/2 drop-shadow-lg lg:shadow-2xl rounded-2xl">
                        ...
                        ?>
                            <a href="<?php echo esc_url($btn_url); ?>" target="<?php echo esc_attr($btn_target); ?>" class="whitespace-nowrap border-none !bg-gradient-to-t from-primary via-secondary to-secondary bg-size-200 bg-pos-100 focus:bg-pos-100  disabled:!bg-[#C9C9C9] [&.disabled]:!bg-[#C9C9C9] disabled:!bg-none [&.disabled]:!bg-none disabled:!opacity-100 [&.disabled]:!opacity-100  !text-white h-[42px] !px-5 xl:!px-12 !rounded-[10px] font-medium items-center justify-center mt-8 transition-all duration-300 hidden group-hover:!flex hover:bg-[length:400px_400px]"><?php echo esc_html($btn_title); ?></a>
                        <?php endif; ?>
                    </div>
                <?php endforeach; ?>
            </div>

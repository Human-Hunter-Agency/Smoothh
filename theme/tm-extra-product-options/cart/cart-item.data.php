...
<div data-js="dropdown">
    <button data-js="dropdown-toggle" class="ml-auto first:mb-2 w-fit whitespace-nowrap border-2 border-[#F2F2F2] hover:border-primary hover:text-primary transition duration-200 text-black min-h-[55px] px-5 rounded-[15px] font-semibold flex items-center justify-center gap-5">
        <?= __('Show configuration', 'smoothh');  ?>
    </button>
    <dl class="tc-epo-metadata variation text-sm p-3 overflow-hidden h-0 transition-all duration-300" data-js="dropdown-container">
        <?php
        if (isset($item_data)) :

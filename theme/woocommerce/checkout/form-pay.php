...
<form id="order_review" method="post">
	...
	<div id="payment" class="not-prose !bg-white p-5 xl:p-8 border border-[#888] !rounded-[15px] mb-20">
		...
		<div class="form-row !p-0">
			...
			<?php echo apply_filters( 'woocommerce_pay_order_button_html', '<button type="submit" class="button alt icon-arrow-right !border-none !bg-gradient-to-b !from-primary !via-secondary !to-secondary bg-size-200 bg-pos-0 hover:bg-pos-100 focus:bg-pos-100 disabled:!bg-[#C9C9C9] disabled:!bg-none disabled:!opacity-100 transition-all duration-200 !text-white whitespace-nowrap !min-h-[55px] !px-5 xl:!px-10 !rounded-[15px] font-bold !flex items-center justify-center gap-5 ' . esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ) . '" id="place_order" value="' . esc_attr( $order_button_text ) . '" data-value="' . esc_attr( $order_button_text ) . '">' . esc_html( $order_button_text ) . '</button>' ); // @codingStandardsIgnoreLine ?>
